...
class Solution:
    def diameterOfBinaryTree1(self, root: Optional[TreeNode]) -> int:
        ...
        def maxfun(node):
            if node is None:
                return -1
            left = maxfun(node.left)
            right = maxfun(node.right)
    ...
    def diameterOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        diameter = [0]
        def maxfun(node, height):
            if node is None:
                return height
            left = maxfun(node.left, height+1)
            right = maxfun(node.right, height+1)
...
s = Solution()
print(s.diameterOfBinaryTree(head))