...
class Solution:
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        ...
        def depth(node, subroot):
            if subroot is None:
                return True
            if node is None:
                return False    
            return (isSame(node, subRoot) if node.val == subRoot.val else False) or depth(node.left, subroot) or depth(node.right, subroot)


        return depth(root, subRoot)    
